...
function updateChUI(){
  ...
  document.getElementById('chtitle-en').textContent=S.bookName+' Chapter '+S.ch;
  ...
  document.getElementById('astat').textContent='வசனத்தை தொட்டு கேளுங்கள்';
}
...
function speakTTS(text,lang,cb){
  ...
  u.onerror=(e)=>{S.playing=false;updPBtn();
    if(e.error!=='interrupted')document.getElementById('astat').textContent=
...
async function playV(i){
  ...
  hlPlay(i);
  const taName=S.bookTaName||S.bookName;
  document.getElementById('atitle').textContent=(S.lang==='ta'?taName:S.bookName)+' '+S.ch+':'+v.num;
  document.getElementById('astat').textContent=(S.lang==='ta'?'வசனம் '+v.num+' இயங்குகிறது...':'Playing verse '+v.num+'...');
...
function seqPlay(){
  if(!S.playAllM||S.pIdx>=S.verses.length){S.playAllM=false;S.playing=false;updPBtn();
    document.getElementById('astat').textContent=(S.lang==='ta'?'முடிந்தது':'Finished');
    return;
  }
  const v=S.verses[S.pIdx];
  hlPlay(S.pIdx);
  document.getElementById('astat').textContent=(S.lang==='ta'?'வசனம் ':'Verse ')+v.num+' / '+S.verses.length;
...
function togPlay(){
  ...
  else if(synth.speaking&&!synth.paused){synth.pause();S.playing=false;updPBtn();document.getElementById('astat').textContent=S.lang==='ta'?'இடைநிறுத்தம்':'Paused';}
  else if(synth.paused){synth.resume();S.playing=true;updPBtn();document.getElementById('astat').textContent=S.lang==='ta'?'இயங்குகிறது...':'Playing...';}
...
function stopAud(){
  ...
  document.querySelectorAll('.vi').forEach(el=>el.classList.remove('vplay'));
  const s=document.getElementById('astat');
